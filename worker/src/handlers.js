import { fetchLeaderboard, generateMessage } from './leaderboard';
...
export async function updateAndNotify() {
  ...
  const oldBoard = JSON.parse(await BOARDS.get('current'));

  await updateBoard(board);
  console.log('Boards updated.');
  await notifySubscribers(oldBoard, board);
 ...
 * @param {*} scheduledDate 
 */
async function notifySubscribers(oldBoard, board) {
  ...
  for (const { name, email, code } of validUsers) {
    const msg = generateMessage(name, oldBoard, board);
    if (msg) {
      const emailRes = await sendEmailTemplate({
        email,
